                            ...
                            <div class="d-flex align-items-center gap-2 mb-2">
                                <div class="rounded-circle bg-primary d-flex align-items-center justify-content-center flex-shrink-0"
                                    ...
                                    @if($announcement->user->avatar)
                                        <img src="{{ $announcement->user->avatar }}" class="w-100 h-100 object-fit-cover">
                                    @else
                                        <span class="material-icons text-white x-small">person</span>
                                    @endif
                                </div>
                                <div>
                        ...
                        @forelse($event->attendees->take(8) as $attendee)
                            <div class="avatar-sm rounded-circle border border-2 border-white shadow-xs overflow-hidden"
                                ...
                                @if($attendee->user->avatar)
                                    <img src="{{ asset($attendee->user->avatar) }}" class="w-100 h-100 object-fit-cover">
                                @else
                                    <div
                                        class="bg-primary text-white w-100 h-100 d-flex align-items-center justify-content-center x-small fw-bold">
                                    ...
                                    @forelse($event->attendees as $attendee)
                                        <div class="d-flex align-items-center py-3 {{ !$loop->last ? 'border-bottom' : '' }}">
                                            ...
                                            <div class="rounded-circle bg-primary bg-opacity-10 d-flex align-items-center justify-content-center me-3" 
                                                ...
                                                @if($attendee->user->avatar)
                                                    <img src="{{ $attendee->user->avatar }}" class="rounded-circle" style="width: 44px; height: 44px; object-fit: cover;">
                                                @else
                                                    <span class="fw-bold text-primary">{{ strtoupper(substr($attendee->user->name, 0, 1)) }}</span>
                                                @endif
                                            </div>
                                            {{-- Info --}}
